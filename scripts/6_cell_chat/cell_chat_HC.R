...
ifnb <- subset(ifnb, subset = disease == "HC")

# Step 1: Extract unique batches
unique_batches <- unique(ifnb@meta.data$batch)

# Step 2: Randomly sample 9 unique batches
set.seed(384)  # Set seed for reproducibility
sampled_batches <- sample(unique_batches, size = 9, replace = FALSE)

# Step 3: Subset the Seurat object to include only the sampled batches
ifnb <- subset(ifnb, subset = batch %in% sampled_batches)
...
saveRDS(cellchat, file = "cellchat_popT_HC1.rds")
