...
# Subset only inhibitory neurons
cells_to_keep <- rownames(allen_seurat@meta.data)[grepl("^Inh", allen_seurat@meta.data$cluster_label)]
allen_seurat_In <- subset(allen_seurat, cells = cells_to_keep)
allen_seurat_In@meta.data$source <- "allen"

rm(allen_seurat)


ifnb = readRDS("hc_rbd_pd_joint_integrated.rds") # already normalized, feature selected, and scaled
ifnb_testct <- subset(ifnb, subset = celltype == "inhibitory neurons")
...
# Integrate
anchors <- FindIntegrationAnchors(object.list = list(ifnb_testct, allen_seurat_In), 
                                   anchor.features = intersect(rownames(ifnb_testct), rownames(allen_seurat_In)))
...
saveRDS(ifnb_testct, file = "integrated_data_Inh.rds")

# Transfer labels from Allen data to your data
transfer_anchors <- FindTransferAnchors(reference = allen_seurat_In, query = ifnb_testct, 
                                        dims = 1:30)
print("Done finding transfer anchors")

predicted_labels <- TransferData(anchorset = transfer_anchors, refdata = allen_seurat_In@meta.data$cluster_label)
ifnb_testct <- AddMetaData(ifnb_testct, metadata = predicted_labels)

saveRDS(ifnb_testct, file = "integrated_ifnb_testct_In.rds")