...
##### Combine raw RNA + ATAC data files + metadata into rds object for HC samples

df <- read.csv("HCTXJ_CTR_PFC.csv", stringsAsFactors = FALSE)
df <- df %>% dplyr::filter(library_type == "Gene Expression")
row_idxes <- which(grepl("^HC", df$sample))
...
# only need first 18 objects for HC-RNA seurat since contains ATAC assay
for (i in 2:18){
    id = ids[i]
...
ifnb <-  merge(data_list[[1]], y = c(data_list[[2]], 
                                     ...
                                     data_list[[17]], 
                                     data_list[[18]]
                                    ), add.cell.ids = sample_names, project = "hc_rna_alldata")

saveRDS(ifnb, file = "hc_rna+atac.rds")
print("Done saving!!")