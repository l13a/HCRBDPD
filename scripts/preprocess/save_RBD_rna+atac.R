...
##### Combine raw RNA + ATAC data files + metadata into rds object for RBD samples

df <- read.csv("HCTXJ_CTR_PFC.csv", stringsAsFactors = FALSE)
df <- df %>% dplyr::filter(library_type == "Gene Expression")
row_idxes <- which(grepl("^RBD", df$sample))
...
ifnb <-  merge(data_list[[1]], y = c(data_list[[2]], 
                                     ...
                                     data_list[[8]], 
                                     data_list[[9]]
                                    ), add.cell.ids = sample_names, project = "rbd_rna_alldata")

saveRDS(ifnb, file = "rbd_rna+atac.rds")
print("Done saving!!")